...
export class ProvinceController {
  ...
  @Get('byDepartament/:idDepartament')
  async listByDepartament(@Param('idDepartament') idDepartament : any){
    try {
      let body:any  = {};

      if(idDepartament){
        body.idDepartament = new mongoose.Types.ObjectId(idDepartament);
      }
  ...
  @Post()
  async create(@Body() body: CreateProvinceDto, @Req() req: Request){
    try {
  ...
  @Put(':id')
  async update(@Param('id')  id : string, @Body() body: UpdateProvinceDto, @Req() req: Request){
    try {
