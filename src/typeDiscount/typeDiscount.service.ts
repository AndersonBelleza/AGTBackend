...
import { CreateTypeDiscountDto } from './dto/createTypeDiscount.dto';
import { UpdateTypeDiscountDto } from './dto/updateTypeDiscount.dto';
import { TypeDiscount } from './typeDiscount.schema';

@Injectable()
export class TypeDiscountService {
  constructor(
    @InjectModel(TypeDiscount.name) private model : Model<TypeDiscount>
  ) {}
  ...
  async create(body : CreateTypeDiscountDto){
    const newRegister = new this.model(body);
    return newRegister.save();
  }

  async update(id: string, body : UpdateTypeDiscountDto){
